export const funcoes = [
  ...
  { type: 'p', text: 'A função é uma regra. Você dá uma entrada, ela aplica a regra, devolve uma saída. A regra nunca muda — a mesma entrada sempre dá a mesma saída.' },
  { type: 'p', text: 'Em matemática, a gente chama a entrada de `x` e escreve a função como `f(x)` — lê-se "f de x". O `f` é só o nome que demos pra regra.' },

  { type: 'h2', text: 'Tipos de função que você vai encontrar' },
  { type: 'p', text: 'Cada tipo de função tem uma forma diferente no gráfico. Aprender a reconhecê-las de cara é mais valioso que decorar fórmulas.' },
  ...
  { type: 'note', text: 'Resumo: função = regra que transforma uma entrada em saída. Cada tipo de função tem um formato de gráfico característico — linear é reta, quadrática é curva em U, exponencial explode pra cima.' },
...
`const f = x => x ** 2;
const g = x => x + 1;
const composta = x => f(g(x));

for (let x = -3; x <= 3; x++) {
  print(\`x=\${x}  |  f(x)=\${f(x)}  |  g(x)=\${g(x)}  |  f(g(x))=\${composta(x)}\`);
}` },
];
